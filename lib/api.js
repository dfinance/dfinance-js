...
const rp = (uri) =>
  typeof uri === "string" ? request({ uri, json: true }) : request(uri);
...
module.exports = class API {
   ...
   */
  getValidators() {
    return rp(this.path("/poa/validators")).then(
   ...
   */
  getAccount(address) {
    return rp(this.path(`/auth/accounts/${address}`)).then(
      (resp) => (resp && resp.result.value) || null
    );
  }
  ...
  getDestroys(page, limit) {
    return rp(
      this.path(`/currencies/withdraws?page=${page}&limit=${limit}`)
